...
class TestHBNBCommand_all(unittest.TestCase):
    ...
    def test_all_objects_space_notation(self):
        cmdline = HBNBCommand()
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(cmdline.onecmd("create BaseModel"))
            self.assertFalse(cmdline.onecmd("create User"))
            self.assertFalse(cmdline.onecmd("create State"))
            self.assertFalse(cmdline.onecmd("create Place"))
            self.assertFalse(cmdline.onecmd("create City"))
            self.assertFalse(cmdline.onecmd("create Amenity"))
            self.assertFalse(cmdline.onecmd("create Review"))

        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(cmdline.onecmd("all"))
            out = output.getvalue().strip()
            print(len(out))
            # self.assertIn("BaseModel", out,
            #               "BaseModel not found in store")
            # self.assertIn("User", output.getvalue().strip(),
            #               "User model not found in store")
            # self.assertIn("State", output.getvalue().strip(),
            #               "State model not found in store")
            # self.assertIn("Place", output.getvalue().strip(),
            #               "Place model not found in store")
            # self.assertIn("City", output.getvalue().strip(),
            #               "City model not found in store")
            # self.assertIn("Amenity", output.getvalue().strip(),
            #               "Amenity not found in store")
            # self.assertIn("Review", output.getvalue().strip(),
            #               "Review not found in store")
            # self.assertFalse(cmdline.onecmd("count Review"))
